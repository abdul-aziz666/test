...
// TestChangeBinaryToDecimal ...
func TestChangeBinaryToDecimal(t *testing.T) {
	tests := []struct {
		name       string
		...
		},
		{
			...
			initRouter: func() (*gin.Context, *httptest.ResponseRecorder) {
				h := httptest.NewRecorder()
				c, _ := gin.CreateTestContext(h)
				c.Request, _ = http.NewRequest(http.MethodGet, "localhost:8085/binary?query=12", nil)
				return c, h
			},
		},
	}
